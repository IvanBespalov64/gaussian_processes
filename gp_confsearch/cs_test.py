...
def calc(dihedrals : list[float]) -> float:
    ...
    en = calc_energy(MOL_FILE_NAME, list(zip(DIHEDRAL_IDS, dihedrals)), NORM_ENERGY, True, RANDOM_DISPLACEMENT)
...
#in kcal/mol!
NORM_ENERGY = calc_energy(MOL_FILE_NAME, [], 0, False, False)
...
dataset = upd_dataset_from_trj("tests/cur_trj.xyz", None)
model.update(dataset)
model.optimize(dataset)

print(f"Current dataset after init:\n{dataset}")

rule = EfficientGlobalOptimization(SparseExpectedImprovement(np.pi / 12))
